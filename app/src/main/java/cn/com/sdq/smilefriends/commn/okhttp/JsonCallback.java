 ...
 * -
 * -如果你对这里的代码原理不清楚，可以看这里的详细原理说明：https://github
 * .com/jeasonlzy/okhttp-OkGo/blob/master/README_JSONCALLBACK.md
 * -如果你对这里的代码原理不清楚，可以看这里的详细原理说明：https://github
 * .com/jeasonlzy/okhttp-OkGo/blob/master/README_JSONCALLBACK.md
 * -如果你对这里的代码原理不清楚，可以看这里的详细原理说明：https://github
 * .com/jeasonlzy/okhttp-OkGo/blob/master/README_JSONCALLBACK.md
...
//                .params("token", "3215sdf13ad1f65asd4f3ads1f");
     ...
     *          @Override
     *          public void onSuccess(LzyResponse<ServerModel> responseData, Call call, Response
     *          response) {
...
//        JsonReader jsonReader = new JsonReader(response.body().charStream());
        String jsonStr = StreamTool.decodeStream(response.body().byteStream());
        //有数据类型，表示有data
        T data = Convert.fromJson(jsonStr, type);

        response.close();
        return data;

        /*//com.lzy.demo.callback.DialogCallback<com.lzy.demo.model.LzyResponse<com.lzy.demo.model
        // .ServerModel>> 得到类的泛型，包括了泛型参数
        Type genType = getClass().getGenericSuperclass();
        ...
        if (typeArgument == Void.class) {
            ...
            //            SimpleResponse simpleResponse = Convert.fromJson(jsonReader,
            // SimpleResponse.class);
            SimpleResponse simpleResponse = Convert.fromJson(jsonStr, SimpleResponse.class);
            response.close();
            ...
            LzyResponse lzyResponse = Convert.fromJson(jsonStr, type);
            response.close();
            int code = lzyResponse.getResult().getCode();
            ...
            if (code == 0) {
                ...
                throw new IllegalStateException("错误代码：" + code + "，错误信息：" + lzyResponse.getResult
                        ().getMsg());
